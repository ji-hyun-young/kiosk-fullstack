...
public record ProductUpsertRequest(String name, String imageUrl, Long price, ProductOption option, ProductType type) {

    public static Product toEntity(ProductUpsertRequest request){
        return Product.builder().name(request.name).imageUrl(request.imageUrl).price(request.price).option(request.option).type(request.type).build();
    }
}
